...
import { initialDoctors, initialServices, initialPatients, initialAppointments, initialOnlineRequests, initialTransactions, initialFollowUps, initialExpenses } from '../data/mockData';
import { getTodayJalaliDate, toEnglishDigits } from '../utils/persianUtils';
const ClinicContext = createContext(undefined);
export const ClinicProvider = ({ children }) => {
    const [scope, setScope] = useState('unified');
    const [activeViewState, setActiveViewState] = useState('dashboard');
    ...
    const [remindersTab, setRemindersTab] = useState('overdue_debts');
    const [appointmentsTab, setAppointmentsTab] = useState('schedule');
    ...
    const [selectedAppointmentForCheckout, setSelectedAppointmentForCheckout] = useState(null);
    ...
    const [isFollowUpResultOpen, setIsFollowUpResultOpen] = useState(false);
    ...
    const openPatientProfile = (patientOrId) => {
    ...
    const checkAppointmentConflict = (date, timeSlot, doctorId, excludeApptId) => {
        const normDate = toEnglishDigits(date).trim().replace(/\//g, '-');
        const normSlot = toEnglishDigits(timeSlot).trim();
        return appointments.some(apt => {
            if (apt.id === excludeApptId || apt.status === 'canceled' || apt.status === 'rescheduled')
                return false;
    ...
    const cancelAppointment = (id, type, reason) => {
        setAppointments(prev => prev.map(apt => {
            if (apt.id === id) {
                // Prevent modifying completed/settled appointments
                if (apt.status === 'completed')
                    return apt;
                const newStatus = type === 'rescheduled' ? 'rescheduled' : 'canceled';
                return {
                    ...apt,
                    status: newStatus,
    ...
    const updateAppointmentPresenceStatus = (id, presenceStatus) => {
        setAppointments(prev => prev.map(apt => {
    ...
    const approveOnlineRequest = (id, confirmedDate, timeSlot, doctorId, customMessage) => {
    ...
    const collectPayment = (patientId, amount, method, posAccount, notes) => {
    ...
    const updateFollowUp = (id, status, resultNote) => {
    ...
    return (_jsx(ClinicContext.Provider, { value: {
            scope,
            setScope,
